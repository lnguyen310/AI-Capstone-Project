...
# loading datasets
# automatically read all smaller master recipe split files
# merges the smaller files to 1 larger one
recipe_files = glob.glob("master_recipes_part*.csv")

# combine into one dataframe
master_df = pd.concat([pd.read_csv(f) for f in recipe_files], ignore_index=True)
